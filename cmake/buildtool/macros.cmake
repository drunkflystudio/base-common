...
macro(generate_project)
    set(options)
    set(one DIRECTORY EXECUTABLE GENERATOR TOOLCHAIN BUILD_TYPE PROJECT_DIR REGISTER_AS)
    set(multi OPTIONS)
    ...
    if(NOT gp_PROJECT_DIR)
        set(gp_PROJECT_DIR "${PROJECT_DIR}")
    endif()

    set(exe_exists FALSE)
    if(NOT gp_EXECUTABLE MATCHES "%CFG%")
        set(exe "${BUILD_DIR}/${gp_DIRECTORY}/${gp_EXECUTABLE}")
        if(EXISTS "${exe}")
            set(exe_exists TRUE)
        endif()
        if(gp_REGISTER_AS)
            list(APPEND EXTRA_CMAKE_OPTIONS "${gp_REGISTER_AS}=${exe}")
        endif()
    else()
        ...
        endforeach()
        if(gp_REGISTER_AS)
            message(FATAL_ERROR "Parameter REGISTER_AS is not supported for multi-config generators.")
        endif()
    endif()
    ...
    if(NOT exe_exists)
        ...
        foreach(opt ${EXTRA_CMAKE_OPTIONS})
            list(APPEND args "-D${opt}")
        endforeach()

        do("${CMAKE_COMMAND}" ${args} "${gp_PROJECT_DIR}"
